...
```sql
SELECT AVG(height),AVG(waist) FROM healthcare WHERE gender = 1 ;
```

```
AVG(height),AVG(waist)
-------------------------------
167.452735422145|86.1579654153583
```

### 10. 키가 가장 큰 사람 중에 두번째로 무거운 사람의 id와 키(height), 몸무게(weight)를 출력하시오.

```sql
SELECT id,height,waist FROM healthcare WHERE height = (SELECT MAX(height) FROM healthcare) ORDER BY weight DESC LIMIT 1 OFFSET 1;
```
