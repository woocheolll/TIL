...
# 표시할 페이지 수 : 7개
def pagination(request, board_objects, contents_num=5):
    # 역순으로 불러오기
    all_boards = board_objects.order_by('-id')
    #get에서 요청받은 페이지 가져오기, 기본값 = 1
    board_page = int(request.GET.get('page',1))
    #페이지당 표시할 게시글 수
    paginator = Paginator(all_boards, contents_num)
    #요청받은 페이지에서 나타낼 게시글들
    boards = pagination.get_page(board_page)
    
    #현재 페이지
    now_page = boards.number
    #마지막 페이지
    end_page = boards.pagination.num_pages
    ...
    if now_page <= 4:
        ...
        for k in range(min_page):
            display_page[k] = (end_page-4)+(k+1)
    
    # 이전 페이지 묶음        
    previous_page_chunk = (now_page-4) + (k+1)
    if previous_page_chunk == now_page-7:
